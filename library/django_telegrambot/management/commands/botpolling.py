...
class Command(BaseCommand):
    ...
    def get_updater(self, username=None, token=None):
        ...
        if username is not None:
            ...
            if not updater:
                self.stderr.write(
                    "Cannot find default bot with username {}".format(username)
                )
        elif token:
    ...
    def handle(self, *args, **options):
        ...
        if settings.DJANGO_TELEGRAMBOT.get("MODE", "WEBHOOK") == "WEBHOOK":
            self.stderr.write(
                "Webhook mode active in settings.py, change in POLLING if you want use polling update"
            )
            return

        updater = self.get_updater(
            username=options.get("username"), token=options.get("token")
        )
        if not updater:
